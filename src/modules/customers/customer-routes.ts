...
const customerContactParamSchema = z.object({
  id: z.string().uuid(),
...
export const registerCustomerRoutes = (
  ...
  app.post(
    '/api/v1/customers',
    { preHandler: guard.requirePermission('customer.create') },
    async (request, reply) => {
      const parsed = createCustomerBodySchema.safeParse(request.body);
  ...
  app.get(
    '/api/v1/customers',
    { preHandler: guard.requirePermission('customer.list') },
    async (request) => {
  ...
  app.get(
    '/api/v1/customers/:id',
    { preHandler: guard.requirePermission('customer.read') },
    async (request) => {
      const parsed = customerIdParamSchema.safeParse(request.params);
  ...
  app.patch(
    '/api/v1/customers/:id',
    { preHandler: guard.requirePermission('customer.update') },
    async (request) => {
      const params = customerIdParamSchema.safeParse(request.params);

      if (!params.success) {
        throw AppError.badRequest('Invalid id');
      }

      const parsed = updateCustomerBodySchema.safeParse(request.body);
  ...
  app.delete(
    '/api/v1/customers/:id',
    { preHandler: guard.requirePermission('customer.delete') },
    async (request, reply) => {
      const params = customerIdParamSchema.safeParse(request.params);
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const customer = await service.addContact(
        params.data.id,
        parsed.data,
      );
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const customer = await service.addNote(
        params.data.id,
        context.employeeId,
        parsed.data.text,
      );
